...
void draw_rectangle(int c, int r)
{
    ...
    {
	printf("%c%c", a, b[2]);
	for(j=0;j<c+2;j++) // R Leght +2추가
		printf(" ");
	printf("%c%c",a, b[2]);
	printf("\n");
    }
    printf("%c%c", a, b[6]);
    for(i=0;i<c+2;i++) // R low +2추가
	printf("%c%c", a, b[1]);
    printf("%c%c", a, b[5]);
    printf("\n");
}
...
void draw_horizontal_slide(int x, int y, int length, char *s) //수 평 
{
	...
	gotoxy(1, y);
	draw_rectangle(real_length*2, 1); // +1 에서*2로변경 
	gotoxy(x+2, y+1);
	printf("%s", s);
...
void draw_vertical_slide(int x, int y, int length, char *s) // 수 직
{
	gotoxy(x, 1);
...
void move_arrow_key(char key, int *x1, int *y1, int x_b, int y_b)
{
	...
	case 72:  //위쪽(상) 방향의 화살표 키 입력
		*y1=*y1-step_v;//1대신에step_v 
		if (*y1<1)	*y1=1; //y좌표의 최소값
		break;
	case 75:  //왼쪽(좌) 방향의 화살표 키 입력
		*x1=*x1-step_h;//1대신에step_h 
